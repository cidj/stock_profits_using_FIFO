...
However, here we take all the trasanction fees into the computation.
...
#The tags are not very well aligned, we have to manually set them. All records
#are read into the dataframe 'records'
cols=['OrderId','Trader','StkCode','Quantity','Price','TradeType','Fee','Date',
'Time']
records=pd.read_table('input4.tsv', skiprows=[0],header=None,names=cols)

#We drop the time information which doesn't affect the caculation. And then the
#records are grouped by the traders' names, stocks' codes and trades' types and
#coverted to records2.
...
#Function stock_benefit is to compute the benefit of some trader. Here the 
#total numbers of buying and selling have to be equal. 
def stock_benefit(buyQ,buyP,sellQ,sellP):
    """ Return the benefit of a given stock. """
    return np.sum(sellQ*sellP)-np.sum(buyQ*buyP)
...
def benefit(buys,buyPrices,buyFees,sells,sellPrices,sellFees):
    ...
    if buyCount>=sellCount :
        ...
        return stock_benefit(res,buyPrices[:edg+1],sells,sellPrices)
    else:
        n=np.cumsum(sells)
        edg=np.where(n>buyCount)[0][0]
        rem=sells[edg]-n[edg]+buyCount
        res=np.hstack((sells[:edg],np.array([rem])))
        return stock_benefit(buys,buyPrices,res,sellPrices[:edg+1])
...
for l1 in np.unique(records2.index.get_level_values('Trader')):
    ...
    for l2 in np.unique(records2.ix[l1].index.get_level_values('StkCode')):
        ...
        theBenefit=benefit(sbuy,sbuyPrice,sbuyFee,ssell,ssellPrice,ssellFee)
        #Collect all the earnings
        earnings=earnings+theBenefit
    #Benefits of every trader are saved into a dictionary. The total transaction
    #fees are deducted from them.
    records3[l1]=earnings-np.sum(records2.ix[l1]['Fee'])
